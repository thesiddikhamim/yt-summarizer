---
name: yt-summarize
description: Summarize youtube content by providing a YouTube URL. Produces a rich Obsidian note with section-by-section breakdowns, wikilinks to all technical concepts and people, and reference notes for every linked term.
user_invocable: true
---
...
The skill ships two person templates in the repo's `templates/` folder (shared with other tools):
...
This keeps interactive runs explicit while letting scheduled tasks / cron / `/loop` pass the mode in the invocation (e.g. `/yt-summarize <url> minimal`) without blocking on input.
